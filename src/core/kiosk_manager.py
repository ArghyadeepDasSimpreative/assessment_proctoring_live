...
class KioskManager:
    ...
    def __init__(self, proctor_socket=None):
        ...
        # Socket client is optional to preserve backward compatibility
    ...
    def _show_security_notification(self):
        ...
        try:
            ...
            self.notification_manager.send(
                title="Skolariq Secure Exam",
                message=(
                    "Your exam session is being destroyed. "
                    "Please resume it again soon, "
                    "unless your session will be terminated."
                ),
            )
    ...
    def _notify_proctor_terminated(self):
        """
        Notify the backend that the native proctoring
        session has been terminated.

        This is intentionally isolated so an unavailable
        socket never breaks the kiosk termination flow.
        """

        if not self.proctor_socket:

            print("[PROCTOR] No socket client configured.")

            return

        try:

            result = self.proctor_socket.notify_proctor_terminated()

            if result:

                print("[PROCTOR] Backend termination " "notification sent.")

            else:

                print(
                    "[PROCTOR] Backend termination " "notification could not be sent."
                )

        except Exception as error:

            print("[PROCTOR] Socket termination " "notification error:", error)
        ...
        while not self.monitor_stop_event.is_set():
            ...
            if self._is_default_desktop_active():
                ...
                if failed_checks >= 2:
                    ...
                    # -------------------------------------------------
                    # 1. Notify backend BEFORE killing the browser.
                    # -------------------------------------------------

                    self._notify_proctor_terminated()

                    # -------------------------------------------------
                    # 2. Immediately kill kiosk browser.
                    # -------------------------------------------------

                    self._kill_edge_process_tree()

                    # -------------------------------------------------
                    # 3. Immediately destroy temporary Edge profile.
                    # -------------------------------------------------

                    self._destroy_session_profile()

                    # -------------------------------------------------
                    # 4. Wait for student to return to normal desktop.
                    # -------------------------------------------------

                    print("[PROCTOR] Waiting for user to return...")

                    while (
                        not self._is_default_desktop_active()
                        and not self.monitor_stop_event.is_set()
                    ):
                    ...
                    # -------------------------------------------------
                    # 5. Show notification after returning.
                    # -------------------------------------------------
